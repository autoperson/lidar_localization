...
bool Matching::InitWithConfig() {
    ...
    std::cout << "-----------------init map matching-------------------" << std::endl;
...
bool Matching::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    std::cout << "the registration method of map matching is ：" << registration_method << std::endl;

    if (registration_method == "NDT") {
        registration_ptr = std::make_shared<NDTRegistration>(config_node[registration_method]);
    } else {
        LOG(ERROR) << "can not find  " << registration_method << ", please check config file!";
        return false;
    }
...
bool Matching::InitFilter(std::string filter_user, std::shared_ptr<CloudFilterInterface>& filter_ptr, const YAML::Node& config_node) {
    ...
    std::cout << "map matching " << filter_user << "'s filter method is：" << filter_mothod << std::endl;
    ...
    if (filter_mothod == "voxel_filter") {
        ...
        LOG(ERROR) << "can not find " << filter_user << "'s " << filter_mothod << ", please check the config file!";
        return false;
    }
