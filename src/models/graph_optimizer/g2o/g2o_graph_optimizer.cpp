...
bool G2oGraphOptimizer::Optimize() {
    ...
    LOG(INFO) << std::endl << "------ the " << ++optimize_cnt << " nd optimize is over -------" << std::endl
              << "node num：" << graph_ptr_->vertices().size() << ", 边数： " << graph_ptr_->edges().size() << std::endl
              << "iterations num： " << iterations << "/" << max_iterations_num_ << std::endl
              << "time：" << optimize_time.toc() << std::endl
              << "error：" << chi2 << "--->" << graph_ptr_->chi2()
              << std::endl << std::endl;
...
//TODO: information matrix of pose as prior
