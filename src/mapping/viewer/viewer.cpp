...
bool Viewer::InitDataPath(const YAML::Node& config_node) {
    ...
    if (!FileManager::CreateDirectory(map_path_, "pointcloud map"))
        return false;
...
bool Viewer::InitFilter(std::string filter_user, std::shared_ptr<CloudFilterInterface>& filter_ptr, const YAML::Node& config_node) {
    ...
    std::cout << "viewer " << filter_user << "'s filter method is：" << filter_mothod << std::endl;
...
bool Viewer::SaveMap() {
    ...
    LOG(INFO) << "the map is saved at：" << std::endl << map_path_ << std::endl << std::endl;
