...
bool BackEnd::InitWithConfig() {
    ...
    std::cout << "-----------------init back end-------------------" << std::endl;
...
bool BackEnd::InitGraphOptimizer(const YAML::Node& config_node) {
    ...
    if (graph_optimizer_type == "g2o") {
        ...
        LOG(ERROR) << "can not find " << graph_optimizer_type << " please check the config file";
        return false;
    }
    std::cout << "the select of optimize is：" << graph_optimizer_type << std::endl << std::endl;
...
bool BackEnd::InitDataPath(const YAML::Node& config_node) {
    ...
    if (!FileManager::InitDirectory(key_frames_path_, "pointcloud of keyframes"))
        return false;
    if (!FileManager::InitDirectory(trajectory_path_, "trajectory files"))
        return false;
...
bool BackEnd::MaybeNewKeyFrame(const CloudData& cloud_data, const PoseData& laser_odom, const PoseData& gnss_odom) {
    ...
    // judge if need new key frame or not
